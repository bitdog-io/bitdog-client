...
function SessionEngine() {
    ...
    this.addCommand('bd-commandResult', this._coreMessageSchemas.commandResultMessageSchema, function (message, configuration, logger) {
        ...
        if (typeof message.d.result !== typeof undefined && message.d.result !== null) {
            if (typeof message.d.result.transactionId !== typeof undefined) {
                if (message.d.result.status === 'error') {
                    _transactionTracker.failed(message.d.result.transactionId, message.d.result);
                }
                else {
                    _transactionTracker.done(message.d.result.transactionId, message.d.result);
                }
            }
        }
...
SessionEngine.prototype.sendMessage = function (message) {
    ...
    if (this._sessionActive === true && this._connectionActive === true) {
        ...
        logger.log(constants.LOG_CATEGORY_MESSAGE, 'received: ', result);
