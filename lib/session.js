...
var getFileAssetUrl = require('./signalRConnection.js').getFileAssetUrl;
...
Session.prototype.getFileAssetUrl = function (resultCallback, authorizationFailedCallback, errorCallback) {
    getFileAssetUrl(resultCallback, authorizationFailedCallback, errorCallback);
};
...
Session.prototype.addCommand = function (name, messageSchema, executeCallback, startCallback, stopCallback, hidden, displayName) {
    return this._sessionEngine.addCommand(name, messageSchema, executeCallback, startCallback, stopCallback, hidden, displayName);
};

Session.prototype.addDataCollector = function (name, messageSchema, intervalMilliseconds, collectCallback, hidden, displayName) {
    return this._sessionEngine.addDataCollector(name, messageSchema, intervalMilliseconds, collectCallback, hidden, displayName);
};
...
function SessionEngine() {
    ...
    if (typeof seconds != typeof undefined && seconds != null && seconds != 0) {
        this.addDataCollector('bd-performance', this._coreMessageSchemas.performanceMessageSchema, 1000 * seconds, this.collectPerformance.bind(this), false,'Performance');
    }
};
...
SessionEngine.prototype.startSession = function () {
    ...
    try {
        ...
        logger.log(constants.LOG_CATEGORY_STATUS, 'Ready event handler exception', e);
    }
...
SessionEngine.prototype.postNodeConfiguration = function (configurationMessage, successCallback, errorCallback) {
    ...
    logger.log(constants.LOG_CATEGORY_STATUS, 'Posting node message schema', configurationMessage.d);
...
SessionEngine.prototype.addCommand = function (name, messageSchema, executeCallback, startCallback, stopCallback, hidden, displayName) {
    return this._commandManager.add(name, messageSchema, executeCallback, startCallback, stopCallback, hidden, displayName);
};

SessionEngine.prototype.addDataCollector = function (name, messageSchema, intervalMilliseconds, collectCallback, hidden, displayName) {
    return this._dataManager.add(name, messageSchema, intervalMilliseconds, collectCallback, hidden, displayName);
};
