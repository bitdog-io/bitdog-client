...
Session.prototype.addCommand = function (name, messageSchema, executeCallback, startCallback, stopCallback) {
    return this._sessionEngine.addCommand(name, messageSchema, executeCallback, startCallback, stopCallback);
};

Session.prototype.addDataCollector = function (name, messageSchema, intervalMilliseconds, collectCallback) {
    return this._sessionEngine.addDataCollector(name, messageSchema, intervalMilliseconds, collectCallback);
};
...
Session.prototype.addQuestion = function (question, answer) {
    return this._sessionEngine.addQuestion(question, answer);
};
...
Session.prototype.sendCommand = function (destinationNodeId, name, messageSchema, callback) {
    return this._sessionEngine.sendCommand(destinationNodeId, name, messageSchema, callback);
};
...
function SessionEngine() {
    ...
    this._connection.on(constants.HUB_NAME, 'routeMessage', this.onRouteMessage.bind(this));
    
    // ErrorResponse Message Handler
    this._connection.on(constants.HUB_NAME, 'ErrorResponse', this.onErrorResponse.bind(this));
    ...
    this.addCommand('bd-updateData',  this._coreMessageSchemas.updateDataMessageSchema, this.onUpdateDataCommand.bind(this));
    this.addDataCollector('bd-performance', this._coreMessageSchemas.performanceMessageSchema, 1000 * 60, this.collectPerformance.bind(this));
};

util.inherits(SessionEngine, EventEmitter);

SessionEngine.prototype.collectPerformance = function (message, configuration, logger) {
    message.freeMemory = os.freemem();
    message.uptime = os.uptime();
    message.loadAverage = os.loadavg();
    message.totalMemory = os.totalmem();
...
SessionEngine.prototype.startSession = function () {
    ...
    if(!configuration.shouldSuppressLogo)
        console.log(constants.LOGOTXT);
    
    logger.log(constants.LOG_CATEGORY_STATUS, 'Connected to Bitdog, welcome! Go to https://bitdog.io to control this node and monitor what it is doing.');

    this._commandManager.start();
    this._dataManager.start();
    this._subscriptionManager.start();
    this._questionManager.start();
    
    this.emit('ready',logger, configuration);

};
   
SessionEngine.prototype.sendQuestionMessage = function (questionText, answerText) {
    var questionMessage = this._coreMessageSchemas.questionMessageSchema.createMessage(constants.MESSAGE_CLASS_QUESTION);
    questionMessage.d.question = questionText;
    questionMessage.d.answer = answerText;
...
SessionEngine.prototype.onSendConfigurationMessage = function (message, configuration, logger) {
    ...
    // Create a single array for message types.
    var messageTypes = new Array();
    
    // Copy the message types into a single message type array
    dataCollectors.copyTo(messageTypes);
    commands.copyTo(messageTypes);
    ...
    messageTypes.forEach(function (item) { configurationMessage.d.messageTypes.push(item); });
...
SessionEngine.prototype.start = function () {
    ...
    if (!this._sessionActive) {
        ...
        logger.log(constants.LOG_CATEGORY_STATUS, "Starting...");
...
SessionEngine.prototype.addCommand = function (name, messageSchema, executeCallback, startCallback, stopCallback) {
    return this._commandManager.add(name, messageSchema, executeCallback, startCallback, stopCallback);
};
    
SessionEngine.prototype.addDataCollector = function (name, messageSchema, intervalMilliseconds, collectCallback) {
    return this._dataManager.add(name, messageSchema, intervalMilliseconds, collectCallback);
};
...
SessionEngine.prototype.addQuestion = function (questionText, answerText) {
    var question = this._questionManager.add(questionText, answerText);
...
SessionEngine.prototype.sendCommand = function (destinationNodeId, name, messageSchema, callback) {
...
SessionEngine.prototype.onUnauthorized = function (response) {
    ...
    logger.log(constants.LOG_CATEGORY_STATUS,'Congratulations! To learn how to activate your new node go to https://bitdog.io/activatenode for more information.'); 
};
...
SessionEngine.prototype.onRouteMessage = function (message) {
    ...
    if (message.h.c == constants.MESSAGE_CLASS_COMMAND) {
        this._commandManager.execute(message);
    }
    else if (message.h.c == constants.MESSAGE_CLASS_DATA) {
