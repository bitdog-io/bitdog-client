...
function SessionEngine() {
    ...
    this.addCommand('bd-commandResult', this._coreMessageSchemas.commandResultMessageSchema, function (message, configuration, logger) {
        logger.log(constants.LOG_CATEGORY_MESSAGE, "Received command result", message);
        if (typeof message.result !== typeof undefined && message.result !== null) {
            if (typeof message.result.transactionId !== typeof undefined) {
                if (message.result.status === 'error') {
                    _transactionTracker.failed(message.result.transactionId, message.result);
                }
                else {
                    _transactionTracker.done(message.result.transactionId, message.result);
                }
            }
        }
...
SessionEngine.prototype.sendMessage = function (message) {
    ...
    if (this._sessionActive === true && this._connectionActive === true) {
        logger.log(constants.LOG_CATEGORY_MESSAGE, 'Invoking server ' + Buffer.byteLength(JSON.stringify(message), 'utf8') + ' byte message: ', message);
        message = this._connection.call(constants.HUB_NAME, 'RouteMessage', message)
     	.done(function (err, resultMessage) {
            if (!err) {
                logger.log(constants.LOG_CATEGORY_MESSAGE, 'Server invocation result: ', resultMessage);
            }
