...
var PingManager = require('./pingManager.js');
...
function SessionEngine() {
    ...
    this._pingManager = new PingManager(this);
    ...
    this.addCommand('bd-ping', this._coreMessageSchemas.pingMessageSchema, function (message, configuration, logger) {
        logger.log(constants.LOG_CATEGORY_CONNECTION_EVENT, "Connection ping received");
    ...
    if (typeof seconds != typeof undefined && seconds != null && seconds != 0) {
        this.addDataCollector({ name: 'bd-performance', displayName: 'Performance' }, this._coreMessageSchemas.performanceMessageSchema, 1000 * seconds, this.collectPerformance.bind(this), false,'Performance');
    }
};
...
SessionEngine.prototype.startSession = function () {
    ...
    this._pingManager.start();

    try {
        this.emit('ready', logger, configuration);
    }
    catch (error) {
        logger.log(constants.LOG_CATEGORY_STATUS, 'Ready event handler exception', error);
    }
    ...
    if (this._macAddress == null) {
        require('getmac').getMac(function (error, macAddress) {
            ...
            if (typeof error === typeof undefined || error === null) {
                ...
                logger.log(constants.LOG_CATEGORY_STATUS, 'Could not get mac address', error);
            }
