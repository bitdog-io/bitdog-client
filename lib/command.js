...
function Command(name, messageSchema, hidden) {
    ...
    this.execute = function (message, configuration, logger) {
        var commandResult = { status: 'success', message: 'ok', transactionId: message.c.x, value: null, destinationId: message.c.n };
    ...
    this.getMessageDefinition = function () {
        return messageSchema.getMessageDefinition(constants.MESSAGE_CLASS_COMMAND, name, _hidden, _displayName);
    };
