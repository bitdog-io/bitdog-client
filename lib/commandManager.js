...
function CommandManager(session) {
    ...
    this.add = function (name, messageSchema, executeCallback, startCallback, stopCallback) {
        var command = new Command(name, messageSchema);
    ...
    this.execute = function (message) {
        if (_isRunning) {
            ...
            if (command == null)
                logger.log(constants.LOG_CATEGORY_ERROR,'Did not find command to execute with id of ' + this.getId(message), message);
            else
                setImmediate(function () { command.execute(message, configuration, logger); });
